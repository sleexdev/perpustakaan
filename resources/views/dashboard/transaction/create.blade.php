     ...
     <form action="/dashboard/transaction" method="post" class="row g-3 col-lg-8">
          ...
          <div class="col-md-6">
               ...
               <select class="form-select @error('kode_member') is-invalid @enderror" name="kode_member" id="kode_member" required autofocus>
                    @foreach ($member as $mb)
                         <option value="{{ $mb->kode_member }}" {{ (collect(old('kode_member'))->contains($mb->kode_member)) ? 'selected':'' }}>{{ $mb->nama }}</option>
                    @endforeach
               </select>
               @error('kode_member')
          ...
          <div class="col-md-6">
               ...
               <select class="form-select @error('book_id') is-invalid @enderror" name="book_id" id="book_id" required>
                    @foreach ($book as $bk)
                         <option value="{{ $bk->id }}" {{ (collect(old('book_id'))->contains($bk->id)) ? 'selected':'' }}>{{ $bk->nama }}</option>
                    @endforeach
               </select>
               @error('book_id')
                    <div class="invalid-feedback">
